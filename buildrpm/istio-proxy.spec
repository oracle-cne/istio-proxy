...
envoy_src_rpm_version=$(repoquery --show-duplicates  istio-envoy-1.30.*  -q --qf "%%{version}" | tail -1)
envoy_src_rpm_nvr=$(repoquery --show-duplicates  istio-envoy-1.30.*  -q --qf "%%{name}-%%{version}-%%{release}" | tail -1)
envoy_src_rpm="istio-envoy-${envoy_src_rpm_version}"
pushd /tmp/envoy-src
yumdownloader --source ${envoy_src_rpm_nvr}
